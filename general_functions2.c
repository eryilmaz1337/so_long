/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   general_functions2.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: eryilmaz <eryilmaz@student.42kocaeli.com.  +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/17 19:27:30 by eryilmaz          #+#    #+#             */
/*   Updated: 2022/05/17 19:27:34 by eryilmaz         ###   ########.tr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

int coin_number(t_game *so_long)
{
	int i;
	int k;
	int c;

	i = 0;
	k = 0;
	c = 0;
	while (so_long->map[i])
	{
		k = 0;
		while (so_long->map[i][k] != '\n')
		{
			
			if (so_long->map[i][k] == 'C')
				c++;
			k++;
		}
		i++;
	}
	return (c);
}

void error_status(t_game *so_long)
{
	if (so_long->index != 2)
		destroy_win_2(so_long, 1,"Lütfen Haritayı düzgün ekleyin tek seferde bir harita eklenebilir");
	if (so_long->fd == -1)
		destroy_win_2(so_long, 1, "**********hata (dosya bulunamadı)**********");
	if(so_long->ik == -1)
		destroy_win_2(so_long, 1, "harita .ber uzantılı olmalıdır");
	if(so_long->hk == -1)
		destroy_win_2(so_long, 1, "!!!!! hata lütfen harita dizaynını kurallara uygun girin !!!!!!!!");
	if(so_long->hfk==-1)
		destroy_win_2(so_long, 1, "Haritada kurallara uymayan kullanımlar var Lütfen Düzeltin");
}

void destroy_win_2(t_game *so_long, int durum, char *str)
{
	printf("%s", str);
	//free(so_long->map);
	//free(so_long->picture);
	//free(so_long->picture->imgplayer);
	//free(so_long);
	exit(durum);
}

img	*insertimgpath()
{
	img *picture;
	picture = malloc(sizeof(img));
	picture->imgplayer = (char **)malloc(sizeof(char *) * 4);
	picture->imgwall = "./img/tree.xpm";
	picture->imgcoin = "./img/potion.xpm";
	picture->imgdoor = "./img/portal.xpm";
	picture->imgground = "./img/floor.xpm";
	picture->imgplayer[0] = "./img/static/down.xpm";
	picture->imgplayer[1] = "./img/static/up.xpm";
	picture->imgplayer[2] = "./img/static/left.xpm";
	picture->imgplayer[3] = "./img/static/right.xpm";
	return (picture);
}